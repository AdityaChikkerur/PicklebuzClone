...
function SortButton({
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-0.5 rounded font-semibold text-muted-foreground hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    >
      {label}
      {active &&
        (dir === "asc" ? (
...
export function RecentMatchesTable() {
  ...
  const matches = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      // Show only the latest 5 matches on the dashboard
        ...
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40 text-xs">
              ...
              <th className="hidden px-4 py-3 md:table-cell sm:px-5">
                Venue
              </th>
            ...
            {loading ? (
              ...
              <tr>
                <td
                  ...
                  className="px-4 py-8 text-center text-sm text-muted-foreground sm:px-5"
                >
                  No verified matches yet. Finish and verify a match to see it
                  here.
                </td>
              </tr>
            ) : (
